...
void clean_up_after_endstop_or_probe_move() {
  ...
  // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
    ...
    bool set_bltouch_deployed(const bool deploy) {
      ...
      // @advi3++: Back port of BLTouch Smart V3 support
        ...
        #if (ENABLED(BLTOUCH_FORCE_5V_MODE) || ENABLED(ENDSTOPPULLUPS)) \
            || (ENABLED(Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN) && ENABLED(ENDSTOPPULLUP_ZMIN)) \
  ...
  float run_z_probe() {
    ...
    // Double-probing does a fast probe followed by a slow probe
      ...
      // @advi3++: If the two measures are too different, there was a problem so return NAN
      const float measured_z =
              abs(z2 - first_probe_z) > 10.0 ? NAN :
              (z2 * 3.0 + first_probe_z * 2.0) * 0.2;
    #else
